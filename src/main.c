...
void save_config(const char *filename, const char *filename2, const char *hostname, struct entries *entries);
...
void start_cb(GtkWidget *widget, gpointer user_data) {
    GtkWidget *image;
    char hostname[32], *filename, *filename2, *remote_host;
    int status;
    ...
    if(gtk_notebook_get_current_page(GTK_NOTEBOOK(notebook)) == 0) {
        if(!server_running) {
            ...
            filename2 = (char *) malloc((strlen(getenv("HOME"))+strlen("/.quicksynergy.conf"))*sizeof(char));
            strcpy(filename2, getenv("HOME"));
            strcat(filename2, "/.quicksynergy.conf");

            save_config(filename, filename2, hostname, (struct entries *) user_data);
...
void load_config(const char *filename, struct entries *entries) {
    FILE *fp;
    char pos[16], host[32];

    if((fp = fopen(filename, "r"))!=NULL) {
        while(!feof(fp)) {
            fscanf(fp, "%s%s", pos, host);

            if(!strcmp(pos, "Above"))
                gtk_entry_set_text(GTK_ENTRY(entries->above_entry), host);
            if(!strcmp(pos, "Right"))
                gtk_entry_set_text(GTK_ENTRY(entries->right_entry), host);
            if(!strcmp(pos, "Below"))
                gtk_entry_set_text(GTK_ENTRY(entries->below_entry), host);
            if(!strcmp(pos, "Left"))
                gtk_entry_set_text(GTK_ENTRY(entries->left_entry), host);
        }
    }
}
...
int main(int argc, char *argv[])
{
    ...
    gtk_image_set_from_pixbuf(GTK_IMAGE(prog_image), gdk_pixbuf_new_from_xpm_data((const char **) logo_data));
    ...
    filename = (char *) malloc((strlen(getenv("HOME"))+strlen("/.quicksynergy.conf"))*sizeof(char));
    strcpy(filename, getenv("HOME"));
    strcat(filename, "/.quicksynergy.conf");

    load_config(filename, &entries);
