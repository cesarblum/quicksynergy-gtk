...
void load_config(struct entry_data *entries) {
	...
	filename = (char *) malloc(
		(strlen(env_home) + strlen("/.quicksynergy/quicksynergy.conf")) * 
        sizeof(char));
		...
		do {
			if(fscanf(f, "%s%s", option, value) != EOF) {
				if(!strcmp(option, "Above"))
					gtk_entry_set_text(
                            GTK_ENTRY(entries->above_entry), value);
				else if(!strcmp(option, "Below"))
					gtk_entry_set_text(
                            GTK_ENTRY(entries->below_entry), value);
				else if(!strcmp(option, "Left"))
					gtk_entry_set_text(
                            GTK_ENTRY(entries->left_entry), value);
				else if(!strcmp(option, "Right"))
					gtk_entry_set_text(
                            GTK_ENTRY(entries->right_entry), value);
                else if(!strcmp(option, "KeepRunning"))
                    keep_running = atoi(value);
			}
		} while(!feof(f));
	}
}
...
void save_config(struct entry_data *entries) {
    ...
    filename = (char *) malloc(
		(strlen(env_home) + strlen("/.quicksynergy")) * sizeof(char));
